...
void HandleResucitate(int UserIndex) {
	...
	if (Npclist[UserList[UserIndex].flags.TargetNPC].NPCtype != eNPCType_Revividor || UserList[UserIndex].flags.Muerto == 0) {
		return;
	}
...
void HandleHeal(int UserIndex) {
	...
	if (Npclist[UserList[UserIndex].flags.TargetNPC].NPCtype != eNPCType_Revividor || !EsNewbie(UserIndex) || UserList[UserIndex].flags.Muerto != 0) {
		return;
	}
