...
void Accion(int UserIndex, int Map, int X, int Y) {
	...
	if (InMapBounds(Map, X, Y)) {
		...
		if (MapData[Map][X][Y].NpcIndex > 0) {
            ...
            } else if (Npclist[tempIndex].NPCtype == eNPCType_Artesano) {

                // Esta el user muerto?
                if (UserList[UserIndex].flags.Muerto == 1) {
                    WriteConsoleMsg(UserIndex, "Estas muerto.", FontTypeNames_FONTTYPE_INFO);
                    return;
                }

                if (UserList[UserIndex].flags.Comerciando == 1) {
                    return;
                }

                if (Distancia(UserList[UserIndex].Pos, Npclist[tempIndex].Pos) > 10) {
					WriteConsoleMsg(UserIndex,"Estas demasiado lejos del artesano.",FontTypeNames_FONTTYPE_INFO);
					return;
				}
